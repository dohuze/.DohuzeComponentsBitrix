...
// 	echo "<pre>arResult[INSTAGRAM_ICONKA_CDN]: "; print_r($arResult["INSTAGRAM_ICONKA_CDN"]); echo "</pre>"; 
// }
...
<? if ($arResult["IS_MOBILE_DEVICE"] != "Y") : ?>
	<div class="anotherInstGallery">
		...
		<div class="anotherInstGallery__img">
			<div class="anotherInstGallery__img__big">
				<? if ($arResult[fotos][media_type_arr][0] == "GraphVideo" || $arResult[fotos][media_type_arr][0] == "VIDEO") : ?>
					<a id="a_0" data-test="<?= $arResult[fotos][media_type_arr][0] ?>" class="instGallery__img_nopoint a_2 a_2Big">
						<video class="videoInst" src="/include/dohuze/instagram_<?= $arResult['ID_COMPONENT'] ?>/<?= $arResult[fotos][dir_foto][0] ?>/0" controlsList="nodownload" disablePictureInPicture></video>
				...
				<? else : ?>
					<a href="<?= $arResult[fotos][data_foto_shortcode_arr][0] ?>" style="background-image: url(&quot;/include/dohuze/instagram_<?= $arResult['ID_COMPONENT'] ?>/<?= $arResult[fotos][dir_foto][0] ?>/0&quot;);" target="_blank" class="a_2 a_2Big">
						<div class="anotherInstGallery__img__big__block">
							<p class="instagram-vidget__description instagram-vidget__date"><?= $arResult[fotos][data_foto_taken_at_timestamp_arr][0] ?></p>
							<p class="mess"><?= $arResult[fotos][data_foto_text_foto_arr][0] ?></p>
						</div>
						<!-- eсли светлый вид, то дополнительно прописываем класс instGallery__likeWhite -->
			...
			<div class="anotherInstGallery__img__small">
				<? for ($i = 1; $i < $arResult[count_foto]; $i++) : ?>
					<? if ($i < 7) : ?>
						<? if (file_exists($_SERVER["DOCUMENT_ROOT"] . '/include/dohuze/instagram_' . $arResult['ID_COMPONENT'] . '/' . $arResult[fotos][dir_foto][$i] . "/$i")) : ?>
							<? if ($arResult[fotos][media_type_arr][$i] == "GraphVideo" || $arResult[fotos][media_type_arr][$i] == "VIDEO") : ?>
								<a id="a_<?= $i ?>" class="instGallery__img_nopoint a_2">
									<video class="videoInst" poster="/include/dohuze/instagram_<?= $arResult['ID_COMPONENT'] ?>/poster_320/<?= $i ?>" controlsList="nodownload" disablePictureInPicture>
							...
							<? else : ?>
								<a href="<?= $arResult[fotos][data_foto_shortcode_arr][$i] ?>" style="background-image: url(&quot;/include/dohuze/instagram_<?= $arResult['ID_COMPONENT'] ?>/<?= $arResult[fotos][dir_foto][$i] ?>/<?= $i ?>&quot;);" target="_blank" class="a_2">
									<div class="anotherInstGallery__img__small__block">
...
<? if ($arResult['count_foto'] > 0) : ?>
	<script>
		...
		for (let i = 0; i < a_2.length; i++) {
			if (a_2[i].classList.contains("a_2Big")) {
				sizeBig = a_2[i].offsetWidth * 18 / 290;
			} else {
				sizeSmall = a_2[i].offsetWidth * 18 / 290;
			}
		}

		for (let el of document.getElementsByClassName("mess")) {
			if (el.parentElement.classList.contains("anotherInstGallery__img__big__block")) {
				el.style.fontSize = sizeBig + "px";
			} else {
				el.style.fontSize = sizeSmall + "px";
			}
		}
		for (let el of document.getElementsByClassName("instagram-vidget__description")) {
			if (el.parentElement.classList.contains("anotherInstGallery__img__big__block")) {
				el.style.fontSize = sizeBig * <?= $arParams['FONT_PERCENT'] ?> + "px";
			} else {
				el.style.fontSize = sizeSmall * <?= $arParams['FONT_PERCENT'] ?> + "px";
			}
		}
	</script>
